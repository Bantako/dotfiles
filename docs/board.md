...
| 4 | **reorg 後パス検証** | NAS 制御 | ser7/NAS 実行。最優先（新ツール前に潰す）。[notes §10 ①](./notes-from-mac.md) |
| 5 | **compose のローカル git** | NAS 制御 | ser7/NAS 実行。`git init` のみ（パイプライン却下）。[notes §10 ②](./notes-from-mac.md) |
| 6 | **lazydocker 接続** | NAS 制御 | ser7 で nhs + ssh/docker context 確認。[notes §10 ③](./notes-from-mac.md) |
