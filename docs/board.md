...
着手順:
1. **`git.yazi` + `glow` opener + `compress.yazi`** — 体感最大
2. **`restore.yazi`** — `trash-cli` 既存。削除キー再定義を伴うので慎重に
3. **`mediainfo.yazi` + `mediainfo` パッケージ** — 唯一の新規 pkg、NAS 動画整理用
...
#### Homepage (ダッシュボード) `[ブロック]`

yaml 設定の minimal UI ダッシュボード。サービス 7+ 個で価値が立つ。
**依存**: [[Navidrome]] 追加後 (10 個近くで最大効用)
詳細: [notes-from-mac.md §9](./notes-from-mac.md)

### コンテンツ軸

#### Navidrome (Subsonic 互換音楽サーバー) `[次]`
...
#### Datasette + dogsheep `[保留]`

Personal Data Warehouse。atuin / GitHub / Paperless / Immich 等を SQLite 横断分析。
**立証条件**: Phase 0 試運転 (30 分、`datasette ~/.local/share/atuin/history.db --port 8081`) で Web UI 文化との折り合いを判定。
**Phase 3 で NAS 統合**: paperless-to-sqlite / immich-to-sqlite が PKM 軸 + コンテンツ軸に接続。
...
Niri が `ext-foreign-toplevel-list-v1` 使用 vs xremap が `wlr-foreign-toplevel-management` でアプリ検出、不整合で `application.only` フィルタが効かない。
**未解決**: ターミナルエミュレータ側完結 案 / Niri IPC daemon 案。
...
| 順 | 項目 | 領域 | 備考 |
|---|---|---|---|
| 0 | **yazi 拡充** (git.yazi / glow opener / compress.yazi) | daily-driver | 管理方法確定済み。dotfiles にファイル追加 + nhs |
| 1 | ~~**jless 追加**~~ | daily-driver | ✓ 完了 |
| 2 | **Navidrome** | NAS コンテンツ | ntfy 完了につき着手可 |
| 3 | **Datasette Phase 0 試運転** | NAS PKM | 30 分の体験で立証可否を決める |
...
improvement-plan.md 完了テーブルへの取り込みは home 側 TODO。simplification-scan の P1-G 表現 (`mapAttrsToList` → `共通属性化`) 修正も同時に。
