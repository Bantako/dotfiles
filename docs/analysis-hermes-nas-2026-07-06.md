...
#### ✅ P1: `GATEWAY_ALLOW_ALL_USERS=true`(修正済み 2026-07-07)

hermes ソースで裏取りした結果、このフラグは **allowlist より優先して全ユーザーを許可**する仕様だった(discord adapter の認可ロジックで最初に評価される)。つまり `DISCORD_ALLOWED_USERS` は無効化されており、bot のいるチャンネルの誰でもエージェントを操作できる状態だった。

**修正**: hermes.nix から `GATEWAY_ALLOW_ALL_USERS` を削除し、allowlist のみで運用に変更。適用・bot 再起動・起動スクリプトからのフラグ消滅を確認済み。ntfy 側は `NTFY_ALLOWED_USERS`(~/.hermes/.env)で引き続き制限される。
...
#### ✅ P3: `config.yaml` の履歴管理(対応済み 2026-07-07)

`~/.hermes` を etckeeper 方式のローカル git リポジトリ化し、hermes-backup タイマー(毎晩 02:30)が自動コミット。追跡対象は config.yaml / SOUL.md / skills / profiles / scripts / cron 定義 / memories(計 ~2010 ファイル)。auth.json / .env / DB / キャッシュ / セッションは `.git/info/exclude` で除外。`.git` ごと NAS→B2 にバックアップされるため、破損時は任意時点の設定に復元でき、エージェントによる設定変更の diff 履歴も追える。
...
#### ✅ P4: user unit の `network-online.target` 依存(修正済み 2026-07-07)

discord bot は起動スクリプト冒頭で discord.com への実疎通を待つ方式に変更し、`StartLimitIntervalSec=300 / Burst=5` + `RestartSec=30s` を設定。WebUI 側は network 依存自体が不要(loopback bind)なので指定を削除。

(以下は当初の指摘)

#### 🟠 P4(原文): user unit の `network-online.target` 依存は効いていない
...
#### ✅ P5: WebUI の生 HTTP 露出(修正済み 2026-07-07)

`127.0.0.1` bind に変更し Tailscale Serve (443/HTTPS) に一本化。loopback 200 / Serve 200 / tailnet 直 8787 拒否を確認済み。

#### ✅ P6: WebUI メモリ制限(設定済み 2026-07-07)

`MemoryHigh=4G` / `MemoryMax=6G` を設定・反映確認済み。

(以下は当初の指摘)

#### 🟠 P5(原文): WebUI が tailnet 全体に生 HTTP (8787) でも露出
...
#### 🟡 P6(原文): WebUI のメモリピーク 8.9GB、リソース制限なし

`systemctl status` でピーク 8.9GB + swap 使用を確認。`MemoryMax=` / `MemoryHigh=` が未設定で、暴走時にデスクトップ全体を巻き込む。`MemoryHigh=4G` 程度の設定を推奨。

#### ✅ P7: 未使用 extras の削減(対応済み 2026-07-07)

17 extras → 5(anthropic / messaging / exa / firecrawl / parallel-web)に削減し、クロージャ **-694MiB**。matrix・voice/tts 系・dingtalk/feishu・modal/daytona・bedrock/azure・fal・hindsight/honcho を削除。daytona は将来のコンテナオーケストレーション導入時の復帰候補としてコメント記載。Discord bot / ntfy / WebUI / Tailscale Serve の動作確認済み。

(以下は当初の指摘)

#### 🟡 P7(原文): 使っていない extras による依存肥大
...
#### ✅ P15: コンテナ異常 → ntfy 通知(対応済み 2026-07-07)

`~/services/monitor/` スタックを新設。`container-alerts`(docker:27-cli + socket read-only)が 5 分間隔で unhealthy / restart policy 付き exited を検出し、**状態が変化したときだけ** `nas-alerts` トピックへ通知(復旧時も 1 回通知)。意図的停止の wger / filebrowser は `IGNORE_PATTERN` で除外。hermes の `hermes-5911e6-2026` トピックとは分離した(エージェントがアラートをコマンド解釈しないため)。

**要対応(人間)**: スマホの ntfy アプリで `nas-alerts` トピックを購読すること。
...
2. ~~`GATEWAY_ALLOW_ALL_USERS` の削除~~(修正済み 2026-07-07)— P1
3. ~~`~/.hermes` のバックアップ追加~~(対応済み 2026-07-06)— P2
4. ~~immich-ml(再起動で解決)+ healthcheck→ntfy 通知~~(対応済み 2026-07-07)— P11, P15
