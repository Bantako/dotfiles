...
以下はMVPへ混ぜず、既存のSupervisor運用がCutover条件を満たした後に派生させる。独立した大型構想として先行実装しない。

### プロジェクトの次の一手と閉じどき

既存task・実行履歴・検証結果から、Supervisorが人間の「次」「続けて」を待たずに次を形成する。
...
初期試験はread-only評価とisolated artifact作成に限定する。行動頻度だけから上位目的や恒久的選好を推定しない。成功はcard作成数ではなく、人間のdispatcher行為が減り、不要な仕事を惰性で延命しなかったかで判定する。

この実験は、新規の仮想projectではなく、Cutover後に実際に進行している既存taskを少数選び、その派生cardとして行う。

### 活動状態に応じた一日の再構成

生活改善ループで得た活動・休養・負担の観測を、Supervisorの仕事量・通知時刻・再評価周期へ接続する。初期段階で人間のCalendarやtaskを自動変更しない。

- Iris自身の再評価時刻を変える
- Workerの起動を延期・縮小する
- 通知を自然な接点まで保留・集約する
- 重い判断を避け、軽い候補へ差し替える
- 変更案と「何もしない」案をShadowで比較する

活動状態から価値や人生目的を推定しない。センサー値を疲労・健康状態の断定に使わず、データ不足時は固定スケジュールへ退化する。人間の予定変更は、生活改善ループ側で安全性と有用性が実測され、別途期限付き委任契約が承認されるまで対象外とする。
